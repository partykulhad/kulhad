...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!requestId) {
      return NextResponse.json(
        { code: 400, message: "Missing required parameter: requestId" },
        { status: 400 }
      );
    }

    // Convert the string requestId to the correct type expected by Convex
    const result = await convex.query(api.canisterCheck.getRequest, { requestId: requestId as any });
